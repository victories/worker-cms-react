...
// Helper: confirm the authed user can act on the given site_id
...
// GET /api/external/sites — every site visible to the owning user, with
// the primary domain joined in. This is the discovery endpoint
// worker-ai-bot calls right after the user pastes their wcms_ key.
...
external.post('/sites/:id/posts', async (c) => {
  ...
  const result = await c.env.DB.prepare(
    `INSERT INTO posts (
       ...
       language, seo_title, seo_description, seo_keywords, published_at
