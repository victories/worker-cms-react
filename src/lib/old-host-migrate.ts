...
export interface MigrateOptions {
  /** Hostname (no scheme) where the original images live, e.g. `eski.example.com`. */
  oldHost: string;
 ...
 * posts often hardcode `https://example.com/storage/foo.png` even
 * though the file lives on `eski.example.com`; we rewrite those at
 * fetch time.
 */
  ...
  for (const post of posts.results || []) {
    ...
    if (setFeatured && (force || post.featured_image_id == null)) {
      ...
      if (candidate) {
        ...
        // Self-referencing URLs (https://example.com/storage/...) get
    ...
    if (rewriteContent && /<img[^>]*?\ssrc=/i.test(workingContent)) {
      ...
      for (const original of srcs) {
        ...
        // Old content often hardcodes `https://example.com/...` even
        // though the file lives on `eski.example.com`. Swap the host
        // before fetching so the request actually finds the file.
        const abs = resolved
          ? rewriteSelfRef(resolved, currentSiteHosts, options.oldHost, scheme)
          : null;
