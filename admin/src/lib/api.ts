...
class ApiClient {
  ...
  async getAiProviderDefinitions() {
    return this.request<{ success: boolean; data: any[] }>('/ai/provider-definitions');
  }

  async getAiProviders() {
    return this.request<{ success: boolean; data: any[] }>('/ai/providers');
  }

  async updateAiProvider(slug: string, data: any) {
    return this.request(`/ai/providers/${slug}`, { method: 'PUT', body: data });
  }

  async getAiProviderModels(slug: string) {
    return this.request<{ success: boolean; data: any[]; error?: string }>(`/ai/providers/${slug}/models`);
  }

  async getAiProviderImageModels(slug: string) {
    return this.request<{ success: boolean; data: any[]; error?: string }>(`/ai/providers/${slug}/image-models`);
  }

  async testAiProvider(slug: string) {
    return this.request<{ success: boolean; data: { success: boolean; error?: string } }>(`/ai/providers/${slug}/test`, { method: 'POST' });
  }

  async deleteAiProvider(slug: string) {
    return this.request(`/ai/providers/${slug}`, { method: 'DELETE' });
  }

  async getAiPrompts() {
    return this.request<{ success: boolean; data: any[] }>('/ai/prompts');
  }

  async getAiPrompt(id: number) {
    return this.request<{ success: boolean; data: any }>(`/ai/prompts/${id}`);
  }

  async createAiPrompt(data: any) {
    return this.request('/ai/prompts', { method: 'POST', body: data });
  }

  async updateAiPrompt(id: number, data: any) {
    return this.request(`/ai/prompts/${id}`, { method: 'PUT', body: data });
  }

  async deleteAiPrompt(id: number) {
    return this.request(`/ai/prompts/${id}`, { method: 'DELETE' });
  }

  async generateAiContent(data: any) {
    return this.request<{ success: boolean; data: any; error?: string }>('/ai/generate', { method: 'POST', body: data });
  }

  async improveAiContent(data: any) {
    return this.request<{ success: boolean; data: any; error?: string }>('/ai/improve', { method: 'POST', body: data });
  }

  async getAiJobs(params: Record<string, string> = {}) {
    const qs = new URLSearchParams(params).toString();
    return this.request<{ success: boolean; data: any[]; meta: any }>(`/ai/jobs${qs ? '?' + qs : ''}`);
  }

  async createAiJob(data: any) {
    return this.request('/ai/jobs', { method: 'POST', body: data });
  }

  async updateAiJob(id: number, data: any) {
    return this.request(`/ai/jobs/${id}`, { method: 'PUT', body: data });
  }

  async deleteAiJob(id: number) {
    return this.request(`/ai/jobs/${id}`, { method: 'DELETE' });
  }

  async retryAiJob(id: number) {
    return this.request(`/ai/jobs/${id}/retry`, { method: 'POST' });
  }

  async getAiLogs(params: Record<string, string> = {}) {
    const qs = new URLSearchParams(params).toString();
    return this.request<{ success: boolean; data: any[]; meta: any }>(`/ai/logs${qs ? '?' + qs : ''}`);
  }

  async getAiLogStats() {
    return this.request<{ success: boolean; data: any }>('/ai/logs/stats');
  }

  // Global AI Settings (super_admin)
  async getGlobalAiProviders() {
    return this.request<{ success: boolean; data: any[] }>('/ai/global/providers');
  }

  async updateGlobalAiProvider(slug: string, data: any) {
    return this.request(`/ai/global/providers/${slug}`, { method: 'PUT', body: data });
  }

  async getGlobalAiProviderModels(slug: string) {
    return this.request<{ success: boolean; data: any[]; error?: string }>(`/ai/global/providers/${slug}/models`);
  }

  async getGlobalAiProviderImageModels(slug: string) {
    return this.request<{ success: boolean; data: any[]; error?: string }>(`/ai/global/providers/${slug}/image-models`);
  }

  async testGlobalAiProvider(slug: string) {
    return this.request<{ success: boolean; data: { success: boolean; error?: string } }>(`/ai/global/providers/${slug}/test`, { method: 'POST' });
  }

  async deleteGlobalAiProvider(slug: string) {
    return this.request(`/ai/global/providers/${slug}`, { method: 'DELETE' });
  }

  async getGlobalAiPrompts() {
    return this.request<{ success: boolean; data: any[] }>('/ai/global/prompts');
  }

  async getGlobalAiPrompt(id: number) {
    return this.request<{ success: boolean; data: any }>(`/ai/global/prompts/${id}`);
  }

  async createGlobalAiPrompt(data: any) {
    return this.request('/ai/global/prompts', { method: 'POST', body: data });
  }

  async updateGlobalAiPrompt(id: number, data: any) {
    return this.request(`/ai/global/prompts/${id}`, { method: 'PUT', body: data });
  }

  async deleteGlobalAiPrompt(id: number) {
    return this.request(`/ai/global/prompts/${id}`, { method: 'DELETE' });
  }
